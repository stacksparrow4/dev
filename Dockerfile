...
RUN apt-get update && apt-get install -y python3 python3-pip sudo gcc git openssl libssl-dev vim php-cli curl wget gdb file netcat-openbsd sslscan nmap binwalk exiftool tmux neovim hydra lftp ftp ncat ffuf gobuster golang unzip zip python3-pwntools python3-ropgadget nasm sagemath strace ltrace elfutils dnsutils
...
RUN /home/user/.local/share/fnm/fnm install --lts

RUN curl -LsSf https://astral.sh/uv/install.sh | sh

RUN cat <<EOF >> /home/user/.bashrc
alias flask-unsign='uvx flask-unsign'
EOF
